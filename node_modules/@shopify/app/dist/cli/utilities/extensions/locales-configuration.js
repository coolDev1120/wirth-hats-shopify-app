...
export async function loadLocalesConfig(extensionPath, extensionIdentifier) {
    ...
    if (defaultLanguageCode.length === 0)
        throw new error.Abort(`Missing default language in ${extensionIdentifier} configuration`, 'Make sure to have a {locale}.default.json file in your locales directory');
    if (defaultLanguageCode.length > 1)
        throw new error.Abort(`Error loading ${extensionIdentifier}`, `There must be one (and only one) locale identified as the default locale: e.g. "en.default.json"`);
    if (totalBundleSize > L10N_BUNDLE_SIZE_LIMIT)
        throw new error.Abort(`Error loading ${extensionIdentifier}`, `Total size of all locale files must be less than ${L10N_BUNDLE_SIZE_LIMIT}`);
    // Locale validations
    for (const locale of localesPaths) {
        const size = fs.statSync(locale).size;
        if (size > L10N_FILE_SIZE_LIMIT)
            throw new error.Abort(`Error loading ${extensionIdentifier}`, `Locale file ${locale} size must be less than ${L10N_FILE_SIZE_LIMIT}`);
        if (size === 0)
