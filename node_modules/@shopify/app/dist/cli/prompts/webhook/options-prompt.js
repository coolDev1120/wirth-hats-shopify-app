 ...
 */
export async function optionsPrompt(flags, availableVersions) {
    const options = {
        topic: '',
        apiVersion: '',
        sharedSecret: '',
    ...
    if (apiVersionPassed) {
        const passedApiVersion = flags.apiVersion.trim();
        if (availableVersions.includes(passedApiVersion)) {
            options.apiVersion = passedApiVersion;
        }
        else {
            throw new error.Abort(`Api Version '${passedApiVersion}' does not exist`, `Allowed values: ${availableVersions.join(', ')}`, ['Try again with a valid api-version value']);
        }
    }
    else {
    ...
    if (methodPassed && !validDeliveryMethodFlag(flags.deliveryMethod)) {
        throw new error.Abort('Invalid Delivery Method passed', `${DELIVERY_METHOD.HTTP}, ${DELIVERY_METHOD.PUBSUB}, and ${DELIVERY_METHOD.EVENTBRIDGE} are allowed`, ['Try again with a valid delivery method']);
    }
    if (methodPassed && addressPassed) {
        if (isAddressAllowedForDeliveryMethod(flags.address, flags.deliveryMethod)) {
            options.address = flags.address.trim();
            options.deliveryMethod = inferMethodFromAddress(options.address);
        }
        else {
            throw new error.Abort("Can't deliver your webhook payload to this address", "Run 'shopify webhook trigger --address=<VALUE>' with a valid URL", deliveryMethodInstructions(flags.deliveryMethod));
        }
    }
    if (!methodPassed && addressPassed) {
        options.address = flags.address.trim();
    ...
    if (methodPassed && !addressPassed) {
        options.address = await addressPrompt(flags.deliveryMethod);
        options.deliveryMethod = inferMethodFromAddress(options.address);
    }
    return options;
}
...
function inferMethodFromAddress(address) {
    ...
    if (method === undefined) {
        throw new error.Abort('No delivery method available for the address', `Use a valid address for ${DELIVERY_METHOD.HTTP}, ${DELIVERY_METHOD.PUBSUB} or ${DELIVERY_METHOD.EVENTBRIDGE}`);
    }
    return method;
}
