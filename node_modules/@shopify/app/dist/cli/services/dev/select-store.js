...
export async function convertToTestStoreIfNeeded(store, org, token) {
    ...
    if (!store.transferDisabled && !store.convertableToPartnerTest) {
        throw new error.Abort(`The store you specified (${store.shopDomain}) is not a dev store`, 'Run dev --reset and select an eligible dev store.');
    }
...
export async function convertStoreToTest(store, orgId, token) {
    ...
    if (!result.convertDevToTestStore.convertedToTestStore) {
        ...
        throw new error.Bug(`Error converting store ${store.shopDomain} to a Test store: ${errors}`, 'This store might not be compatible with draft apps, please try a different store');
    }
