...
export async function ensureDevEnvironment(options, token) {
    ...
    if (!organization)
        throw new error.Bug(`Couldn't find Organization with id ${orgId}.`);
    if (!selectedApp) {
        if (cachedInfo?.appId) {
            const app = await fetchAppFromApiKey(cachedInfo.appId, token);
            if (!app)
                throw new error.Bug(`Couldn't find App with apiKey ${cachedInfo.appId}.`);
            selectedApp = app;
        }
        else {
...
export async function fetchAppAndIdentifiers(options, token) {
    ...
    if (options.reset) {
        ...
        if (!partnersApp) {
            throw new kitError.Abort(output.content `Couldn't find the app with API key ${apiKey}`, output.content `• If you didn't intend to select this app, run ${output.content `${output.token.packagejsonScript(options.app.packageManager, 'deploy', '--reset')}`.value}`);
        }
    }
    else {
...
async function fetchDevDataFromOptions(options, orgId, token) {
    ...
    if (options.storeFqdn) {
        ...
        if (!orgWithStore)
            throw new error.Bug(`Could not find Organization for id ${orgId}.`);
        if (!orgWithStore.store) {
            const partners = await partnersFqdn();
            const org = orgWithStore.organization;
            throw new error.Bug(`Could not find ${options.storeFqdn} in the Organization ${org.businessName} as a valid development store.`, `Visit https://${partners}/${org.id}/stores to create a new store in your organization`);
        }
