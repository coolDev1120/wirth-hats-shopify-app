...
import { optionsPrompt } from '../../prompts/webhook/options-prompt.js';
...
export async function webhookTriggerService(flags) {
    ...
    const sample = await getWebhookSample(token, options.topic, options.apiVersion, options.deliveryMethod, options.address, options.sharedSecret);
    if (!sample.success) {
