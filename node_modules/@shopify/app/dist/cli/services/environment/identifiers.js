...
function handleIdsError(errorType, appName, packageManager) {
    switch (errorType) {
        ...
        case 'invalid-environment':
            throw new error.Abort(`Deployment failed because this local project doesn't seem to match the app "${appName}" in Shopify Partners.`, `If you didn't intend to select this app, run ${output.content `${output.token.packagejsonScript(packageManager, 'deploy', '--reset')}`.value}
